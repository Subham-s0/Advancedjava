...
public class UserDaoimpl implements UserDao {
		...
		try (Connection conn = DBconnection.getDbConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				if (rs.next()) {
					...
					user = new usermodel(rs.getString("user_id"), rs.getString("user_firstname"),
							rs.getString("user_lastname"), rs.getString("user_email"), rs.getString("user_name"),
							rs.getDate("user_dob").toString(), rs.getString("user_phnno"), genderEnum,
							rs.getString("user_password"), rs.getString("role_type"), rs.getString("user_profile"),
	...
	public int saveUser(usermodel user) throws DataAccessException {
		...
		try {
			...
			String sql = "INSERT INTO users (user_firstname, user_lastname, user_email, "
					+ "user_name, user_dob, user_phnno, user_password) " + "VALUES (?, ?, ?, ?, ?, ?, ?)";
	...
	public List<usermodel> getAllUsers() throws DataAccessException {
		...
		try (Connection conn = DBconnection.getDbConnection();
			...
			while (rs.next()) {
				...
				usermodel user = new usermodel(rs.getString("user_id"), rs.getString("user_firstname"),
						rs.getString("user_lastname"), rs.getString("user_email"), rs.getString("user_name"),
						rs.getDate("user_dob").toString(), rs.getString("user_phnno"), genderEnum,
						rs.getString("user_password"), rs.getString("role_type"), rs.getString("user_profile"),
						userStatus.valueOf(rs.getString("user_status").toLowerCase()));
	...
	public int getTotalUsers() throws DataAccessException {
		...
		try (Connection conn = DBconnection.getDbConnection();
			...
			if (rs.next()) {
				return rs.getInt(1); // Gets the COUNT(*) result
			} else {
				return 0; // Fallback if no result (shouldn't normally happen)
			}
		} catch (SQLException e) {
			throw new DataAccessException("Error fetching total number of users", e);
		}
	}
