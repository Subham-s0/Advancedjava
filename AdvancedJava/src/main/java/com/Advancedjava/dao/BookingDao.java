...
public class BookingDao {
	...
	public List<BookingModel> getBookingsByUserId(String id) throws DataAccessException {
		...
		try (Connection conn = DBconnection.getDbConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {

			ps.setString(1, id);

			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					BookingModel booking = new BookingModel(rs.getInt("booking_id"), rs.getInt("property_id"),
							rs.getInt("user_id"), BookingStatus.valueOf(rs.getString("booking_status")),
							rs.getDate("check_in_date"), rs.getDate("check_out_date"), rs.getBigDecimal("base_price"),
							rs.getBigDecimal("total_price"), rs.getInt("discount_percent"),
							rs.getInt("number_of_guests"), rs.getTimestamp("booking_created_at"));
	...
	public int getTotalBookings() throws DataAccessException {
		...
		try (Connection conn = DBconnection.getDbConnection();
			...
			if (rs.next()) {
				return rs.getInt(1); // Total number of bookings
			} else {
				return 0; // Shouldn't happen, but fallback
			}

		} catch (SQLException e) {
			throw new DataAccessException("Error fetching total booking count", e);
		}
	}

	public List<CategoryBookingCount> getBookingCountByCategory() throws DataAccessException {
		List<CategoryBookingCount> categoryCounts = new ArrayList<>();

		String sql = "SELECT c.category_name, COUNT(b.booking_id) AS total_bookings " + "FROM booking b "
		...
		try (Connection conn = DBconnection.getDbConnection();
			...
			while (rs.next()) {
				...
				categoryCounts.add(new CategoryBookingCount(categoryName, totalBookings));
			}

		} catch (SQLException e) {
			throw new DataAccessException("Error getting bookings by category", e);
		}
