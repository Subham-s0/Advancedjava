...
public class AmenityDaoImpl implements AmenityDao {
	...
	public List<AmenityModel> findAll() throws DataAccessException {
		...
		try (Connection conn = DBconnection.getDbConnection();
			...
			while (rs.next()) {
				AmenityModel amenity = new AmenityModel(rs.getInt("amenity_id"), rs.getString("amenity_name"),
						rs.getString("amenity_description"), rs.getString("amenity_icon"));
	...
	public AmenityModel findById(int amenityId) throws DataAccessException {
		...
		try (Connection conn = DBconnection.getDbConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				if (rs.next()) {
					amenity = new AmenityModel(rs.getInt("amenity_id"), rs.getString("amenity_name"),
							rs.getString("amenity_description"), rs.getString("amenity_icon"));
	...
	public boolean deletePropertyAmenity(int propertyId) throws DataAccessException {
		String sql = "DELETE FROM property_amenity WHERE property_id = ?";
		...
		try (Connection conn = DBconnection.getDbConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			throw new DataAccessException("Error deleting property amenities by property ID", ex);
		}
	}
}
