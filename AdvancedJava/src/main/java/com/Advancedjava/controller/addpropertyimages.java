		...
		if (propertyIdStr == null || propertyIdStr.trim().isEmpty() || "null".equalsIgnoreCase(propertyIdStr)) {
			...
			if (errors.isEmpty()) {
				...
				try {
					...
					request.getRequestDispatcher("WEB-INF/pages/admin/addpropertyimages.jsp").forward(request,
							response);
				}
				...
				for (int i = 0; i < imageParts.size(); i++) {
					...
					try {
						...
						boolean duplicateFileName = existingImages.stream()
								.anyMatch(img -> img.getFileName().equalsIgnoreCase(customFileName)); // ✅ Compare
																										// against
																										// custom
																										// filename

						if (duplicateFileName) {
							errors.add("File already exists: " + originalFileName); // 🚨 Show original name, not
																					// sanitized
							continue;
						}

						boolean uploaded = imageUtil.uploadImage(imagePart, "", saveFolder, customFileName);
