...
public class UpdateProfileService {
	...
	public Boolean UpdateProfile(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			} else if (!email.equalsIgnoreCase(currentUser.getUserEmail()) && userDao.existsByEmail(email)) {
			...
			} else if (!username.equalsIgnoreCase(currentUser.getUserName()) && userDao.existsByUsername(username)) {
			...
			} else if (!phone.equals(currentUser.getUserPhnNo()) && userDao.existsByPhone(phone)) {
			...
			if (errors.isEmpty() && imageUploaded) {
				usermodel updatedUser = new usermodel(currentUser.getUserId(), firstName, lastName, email, username,
						dob, phone, genderEnum, currentUser.getUserPassword(), currentUser.getuserRole(),
						customFilename, currentUser.getUserStatus());
	...
	public Boolean ChangePassword(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (errorPassword.isEmpty()) {
				...
				usermodel updatedUser = new usermodel(currentUser.getUserId(), currentUser.getUserFirstName(),
						currentUser.getUserLastName(), currentUser.getUserEmail(), currentUser.getUserName(),
						currentUser.getUserDob(), currentUser.getUserPhnNo(), currentUser.getGender(), hashed,
						currentUser.getuserRole(), currentUser.getUserProfilePicture(), currentUser.getUserStatus());
