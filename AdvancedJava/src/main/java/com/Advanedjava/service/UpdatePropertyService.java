...
public class UpdatePropertyService {
	public Boolean Updatedetails(HttpServletRequest request, HttpServletResponse response, int PropertyId)
		...
		if (!errors.isEmpty()) {
			try {
				...
				request.getRequestDispatcher("WEB-INF/pages/admin/editpropertydashboard.jsp").forward(request,
						response);
				;
			} catch (Exception e) {
		...
		try {
			Propertymodel property = new Propertymodel(PropertyId, name, address, city, country, description,
